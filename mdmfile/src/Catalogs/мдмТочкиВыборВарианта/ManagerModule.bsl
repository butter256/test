...
Функция СоздатьОбновитьТочки(Точка, Элемент, БизнесПроцесс) Экспорт
	...
	Иначе 
		Объект = Справочники.мдмТочкиВыборВарианта.СоздатьЭлемент();
	...
	Для Каждого Вариант из Элемент.Элементы цикл
		НовыйВариант = Объект.Варианты.Добавить();
		НовыйВариант.Имя = Вариант.Значение;
		НовыйВариант.Наименование = Вариант.Наименование;
	КонецЦикла;	
	...
	Для Каждого Связь из ТаблицаСвязей цикл
		...
		Если Связь.Начало = Точка.Имя тогда
			...
			Если ЗначениеЗаполнено(ТекущаяТочка) тогда
				...
				НоваяСтрока.Результат = Связь.НачалоВариант;
			КонецЕсли;
...
Функция ЭтотОбъект() Экспорт
	...
	Возврат Справочники.мдмТочкиВыборВарианта;
